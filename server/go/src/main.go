...
func main() {
	...
	router.GET("/gameplay/session/:id", auth.WithPlayerOptional, s.GetOneSession)
	router.GET("/gameplay/session/:id/scoreboard", auth.WithPlayerOptional, s.GetSessionScoreboard)
	router.GET("/gameplay/session/:id/score-history", auth.WithPlayerOptional, s.GetSessionScoreHistory)
	router.POST("/gameplay/session", s.CreateSession)
	router.PUT("/gameplay/session/:id", s.WithValidSession, auth.WithPlayer, s.AsMod, s.UpdateSession)
	router.DELETE("/gameplay/session/:id", s.WithValidSession, auth.WithPlayer, s.AsMod, s.DeleteSession)
	router.GET("/gameplay/session/:id/players", auth.WithPlayerOptional, s.GetPlayersInSession)

	fmt.Println("\nAdministration API:")
	router.POST("/gameplay/session/:id/start", s.WithValidSession, auth.WithPlayer, s.AsMod, s.StartSession)
	router.PUT("/gameplay/session/:id/current-round", s.WithValidSession, auth.WithPlayer, s.AsMod, s.SetCurrentRound)
	router.GET("/gameplay/session/:id/current-round", s.GetCurrentRound)
	router.PUT("/gameplay/session/:id/current-question", s.WithValidSession, auth.WithPlayer, s.AsMod, s.SetCurrentQuestion)
	router.GET("/gameplay/session/:id/current-question", auth.WithPlayerOptional, s.GetCurrentQuestion)
	router.PUT("/gameplay/session/:id/score", s.WithValidSession, auth.WithPlayer, s.AsMod, s.ScoreQuestion)
	router.GET("/gameplay/session/:id/wagers", auth.WithPlayerOptional, s.GetWagers)
	router.GET("/gameplay/session/:id/state", s.GetSessionState)

	fmt.Println("\nAnswer API:")
	router.POST("/gameplay/session/:id/answer", auth.WithPlayer, s.AnswerQuestion)
	router.GET("/gameplay/session/:id/answers", auth.WithPlayerOptional, s.GetAnswers)
	...
	router.PUT("/gameplay/session/:id/hot-edit-round-name", s.WithValidSession, auth.WithPlayer, s.AsMod, s.HotEditRoundName)
