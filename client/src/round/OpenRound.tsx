...
interface Props {
    ...
    questions: string[]
    wagers: number[]
    set: (round_id: string, key: string, value: any, save: boolean) => void
    set_selected: (id: string) => void
    delete: (id: string) => void
    save: (id: string) => void
    set_multi: (round_id: string, update: any, save: boolean) => void
    token: string
}

class OpenRound extends React.Component<Props> {

    set_name = (event: React.ChangeEvent<HTMLInputElement>) => {
        this.props.set(this.props.id, NAME, event.target.value, false)
    }

    set_wager = (index: number, value: number, save: boolean) => {
    ...
    add_questions = (questions_list: string[]) => {
        const new_wagers = []
    ...
    remove_questions = (questions_list: string[]) => {
        ...
        return (
            <div className="open-round">
                ...
                <Collapse defaultActiveKey={['1', '2']}
                    style={{ width: "100%", 'border-left': 0, 'border-right': 0 } as any}>
